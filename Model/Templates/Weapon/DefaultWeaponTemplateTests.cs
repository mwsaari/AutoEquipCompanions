...
namespace AutoEquipCompanions.Test.Model.Templates.Weapon
{
   public class DefaultWeaponTemplateTests
   {
      ...
      {
         // No bastard-sword exception here: difficulty is gated on the item's own RelevantSkill
      ...
      [Fact]
      public void GetScore_ReturnsItemValue()
      {
         var el = Helpers.MakeWeapon(ItemObject.ItemTypeEnum.OneHandedWeapon);
         Assert.Equal(el.ItemValue, DefaultWeaponTemplate.Instance.GetScore(el));
      }

      // ── IsBetterThan ────────────────────────────────────────────────────────

      [Fact]
      public void IsBetterThan_EqualScore_ReturnsFalse()
      {
         var el = Helpers.MakeWeapon(ItemObject.ItemTypeEnum.OneHandedWeapon);
         Assert.False(DefaultWeaponTemplate.Instance.IsBetterThan(el, el));
      }

      [Fact]
      public void IsBetterThan_BothEmpty_ReturnsFalse()
      {
         Assert.False(DefaultWeaponTemplate.Instance.IsBetterThan(default, default));
      }
   }
}
